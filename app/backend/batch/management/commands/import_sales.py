...
def execute(download_history):
  ...
  if entry.status != Status.ASINC_UNPROCESSED:
    return
  ...
  entry.status = Status.ASINC_PROCESSED
  entry.save()
...
class Command(BaseCommand):
  def handle(self, *args, **options):
    while True:
      download_history = SalesFile.objects.filter(status=Status.ASINC_UNPROCESSED).order_by('id').first()
